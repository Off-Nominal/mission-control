...
export async function setSubscriptions(
  ...
  if (subCommand === "subscribe") {
    const newEventInput = interaction.options.getBoolean("new-event");
    newEvent = newEventInput === null ? undefined : newEventInput;
    preEvent = interaction.options.getInteger("pre-event") || undefined;
  }
  ...
  // try {
  //   const userSettings = await userNotifications.setEventSubscriptions(
  //     interaction.user.id,
  //     newEvent,
  //     preEvent
  //   );
