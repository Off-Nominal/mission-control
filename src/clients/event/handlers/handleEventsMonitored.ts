...
export default function generateHandleEventsMonitored(db) {
  const { fetchPreNotificationSubscribers } = userQueries(db);

  return async function handleEventsMonitored(eventWindows: EventWindow[]) {
