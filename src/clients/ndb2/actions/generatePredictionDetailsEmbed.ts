...
const getOddsMessage = (endorsePayout: number, undorsePayout: number) => {
  return `A succesful prediction would pay current endorsers at ${endorsePayout} times their wager (days). Undorsers woud lose that many points.\n\nA failed prediction would pay out current undorsers at ${undorsePayout} their wager (days), and endorsers would lose that many.`;
};
